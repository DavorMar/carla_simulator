...
class ReplayBuffer:
    def __init__(self, max_size, input_shape, n_actions, name = "memory", chkpt_dir = "tmp\memory_td3"):
    ...
    def save_memory(self):
        ...
        np.save(fr"{self.chkpt_dir}\{self.file_name}-state", self.state_memory[:memory_len])
        np.save(fr"{self.chkpt_dir}\{self.file_name}-states_", self.new_state_memory[:memory_len])
        np.save(fr"{self.chkpt_dir}\{self.file_name}-actions", self.action_memory[:memory_len])
        np.save(fr"{self.chkpt_dir}\{self.file_name}-rewards", self.reward_memory[:memory_len])
        np.save(fr"{self.chkpt_dir}\{self.file_name}-dones", self.terminal_memory[:memory_len])


    def load_memory(self):
        state_memory = np.load(fr"{self.chkpt_dir}\{self.file_name}-state.npy")
        new_state_memory = np.load(fr"{self.chkpt_dir}\{self.file_name}-states_.npy")
        ...
        self.reward_memory[:mem_cntr] = reward_memory
        self.terminal_memory[:mem_cntr] = terminal_memory
        self.mem_cntr = mem_cntr
        print("loaded memory ", self.mem_cntr)
...
class CriticNetwork(keras.Model):
    def __init__(self, fc1_dims, fc2_dims, name, chkpt_dir='tmp/td3'):
        ...
        self.fc1 = Dense(self.fc1_dims, activation='relu', input_shape=(14,))#CHECK IF IT IS 14
...
class ActorNetwork(keras.Model):
    def __init__(self, fc1_dims, fc2_dims, n_actions, name,
        ...
        self.fc1 = Dense(self.fc1_dims, activation='relu', input_shape=(11,))
        self.fc2 = Dense(self.fc2_dims, activation='relu')
...
class Agent:
    def __init__(self, alpha, beta, input_dims, tau, env,
        ...
        self.actor.compile(optimizer=adam_v2.Adam(learning_rate=alpha), loss='mean')
        self.critic_1.compile(optimizer=adam_v2.Adam(learning_rate=beta),
                              loss='mean_squared_error')
        self.critic_2.compile(optimizer=adam_v2.Adam(learning_rate=beta),
                              loss='mean_squared_error')

        self.target_actor.compile(optimizer=adam_v2.Adam(learning_rate=alpha),
                                  loss='mean')
        self.target_critic_1.compile(optimizer=adam_v2.Adam(learning_rate=beta),
                                     loss='mean_squared_error')
        self.target_critic_2.compile(optimizer=adam_v2.Adam(learning_rate=beta),
                                     loss='mean_squared_error')
